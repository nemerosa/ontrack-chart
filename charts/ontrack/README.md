...
![Version: 5.0.16](https://img.shields.io/badge/Version-5.0.16-informational?style=flat-square) ![Type: application](https://img.shields.io/badge/Type-application-informational?style=flat-square) ![AppVersion: 5.0.4](https://img.shields.io/badge/AppVersion-5.0.4-informational?style=flat-square)
...
| auth.keycloak.ldap.tls | object | `{"enabled":false,"secretName":"ontrack-keycloak-ldap-tls"}` | LDAP TLS configuration |
| auth.keycloak.ldap.tls.enabled | bool | `false` | If enabled, the LDAP connection will use TLS |
| auth.keycloak.ldap.tls.secretName | string | `"ontrack-keycloak-ldap-tls"` | Secret name containing the TLS certificates The secret is expected to contain the following keys: * ca.crt * tls.crt * tls.key |
...
| initContainers | object | `{"image":{"repository":"busybox","tag":"1.37.0"},"jreImage":{"repository":"eclipse-temurin","tag":"17-jre-alpine"}}` | General configuration of the init containers |
...
| initContainers.jreImage | object | `{"repository":"eclipse-temurin","tag":"17-jre-alpine"}` | Configuration of the image used for the init containers requiring a JRE (like Keycloak LDAP TLS) |
| initContainers.jreImage.repository | string | `"eclipse-temurin"` | Image repository (including the registry) |
