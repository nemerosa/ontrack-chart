...
| auth.keycloak.settings | object | `{"admin":{"email":"","firstName":"Admin","lastName":"User","password":"admin","username":"admin"},"enabled":true,"registrationAllowed":true,"resetPasswordAllowed":true}` | Provisioning settings |
...
| ingress.enabled | bool | `false` | Creating an Ingress for the Ontrack different services |
...
| ontrack.config.license.embedded.key | string | `""` | Provided license key |
| ontrack.config.license.fixed.active | bool | `false` | Is the license active? |
| ontrack.config.license.fixed.assignee | string | `nil` | Name of the assignee |
| ontrack.config.license.fixed.maxProjects | int | `0` | Maximum number of projects which can be created (0 for unlimited) |
| ontrack.config.license.fixed.name | string | `nil` | Type of license, display name, description, etc. |
| ontrack.config.license.fixed.validUntil | string | `nil` | End of validity for this license (null for unlimited) |
| ontrack.config.license.stripe.secret | string | `"ontrack-license-stripe"` | Name of the secret which contains the Stripe information |
| ontrack.config.license.stripe.subscription_key_name | string | `"subscription"` | Name of the secret key which contains ID of the Stripe subscription |
| ontrack.config.license.stripe.token_key_name | string | `"token"` | Name of the secret key which contains the API key |
| ontrack.config.license.type | string | `"none"` | Type of license: none, stripe, fixed |
